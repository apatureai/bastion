 ...
 * `{ state: "failed", error: "canceled" }`, so Bastion must map that exact
 ...
 * pins every row, so a change to Verdict's cancel surfacing (e.g. a
...
/**
 * The sentinel error string Verdict's public poll uses to disclose a
...
/** How Verdict's public poll reports a job (the shape Bastion reads). */
...
export interface MappedEngineStatus {
   ...
   * (`cancelling`) is NON-terminal and stays externally `running`. Bastion
...
/**
 * Map a Verdict poll status to the MCP-visible job status. Pure and
