...
**Migration arbitration added July 14, 2026 (#52).** Every production boot enters one transaction, acquires the product-scoped Postgres advisory lock, and only then rereads migration state. The lock, all pending DDL, and their tracking rows share that transaction, so concurrent replicas serialize and a crash rolls back before the lock is automatically released. Applied IDs carry SHA-256 checksums: legacy ID-only rows are adopted once from the matching image, known historical files are immutable afterward, and a mismatch fails startup. An older additive-compatible image may observe an unknown newer checksum-pinned migration without rewriting it, which preserves rolling rollback while still validating every migration that image knows.

**Cancellation binding added July 12, 2026 (#32).** The application record persists the distinct Judgment Engine job ID, first cancellation timestamp/reason, and arbitration decision. Production cancel/poll calls use that engine ID, never the MCP product ID. OAuth `reviews:cancel`, tenant, and principal ownership are checked before state is read or changed; MCP transport sessions are likewise bound to both tenant and principal. Completion and cancellation share one store transaction as their terminal linearization point, so a late result cannot overwrite a winning cancellation.
