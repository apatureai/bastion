...
This repository is developed on `main`. The current release is `v0.1.1`, published to npm as
`@apatureai/bastion` and `@apatureai/bastion-types`. `main` is the supported version: fixes land
there first, and anyone running this code should track it rather than pinning a tag.
